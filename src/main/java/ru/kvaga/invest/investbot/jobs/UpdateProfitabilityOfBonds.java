...
//			BondItemForProfitabilityPrinting bifpp = new BondItemForProfitabilityPrinting();
			...
			HashMap<String,String> mapOfTickersAndBondNames = BondsTrackingLib.getListOfBondsFromURL(urlOfForeignBondsList);
			...
			for(String ticker : mapOfTickersAndBondNames.keySet()) {
				try {
					...
					listOfBonds.add(new BondItemForProfitabilityPrinting(ticker, mapOfTickersAndBondNames.get(ticker), BondsTrackingLib.bondProfitability(BondsTrackingLib.getInfoFromHTMLForCalculationProfitability(html), StocksTrackingLib.getCurrentPriceOfStock("", html, ""))));
				}catch(Exception e) {
					log.error("Couldn't get info for calculation profitability for ticker ["+ticker+"]", e);
					App.telegramSendMessage.sendMessage("BondException: <a href='" + String.format(ConfigMap.URL_TEXT_TINKOFF_BONDS, ticker) + "'>"
							+ URLEncoder.encode(mapOfTickersAndBondNames.get(ticker),StandardCharsets.UTF_8.toString()) + "</a> ");
				}
			}
...
//				App.telegramSendMessage.sendMessage(URLEncoder.encode(message,StandardCharsets.UTF_8.toString()));
				...
				log.info(
						"Message for bond " + bifpp.getBondName() + " " + bifpp.getTicker() + " was sent to telegram");
			} catch (Exception e) {
				if (e.getMessage().contains("Server returned HTTP response code: 429 for URL")) {
					...
					log.debug("Trying to resend message about bond " + bifpp.getBondName() + " " + bifpp.getTicker());
					try {
						App.telegramSendMessage.sendMessage(message);
